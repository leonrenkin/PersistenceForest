...
def _validate_inputs(
    ...
    if class_name != "clean" and tendril_length <= 0.0:
...
def generate_loop_tendrils(
    ...
    """Generate one loop-with-inward-tendrils graph."""
    if class_name not in SUPPORTED_CLASSES:
        raise ValueError(
            f"Unknown class_name {class_name!r}. "
            f"Supported values are: {', '.join(SUPPORTED_CLASSES)}"
    ...
    if class_name == "polarized":
        ...
        if allowed_indices.size < effective_count:
            notes.append("polarized sector had fewer roots than requested; root selection was relaxed")

    if effective_count * max(1, min_root_separation) > n_loop_vertices:
    ...
    labels: dict[str, object] = {
        ...
        "n_tendrils": int(effective_count),
        "tendril_length": float(effective_length),
        "segments_per_tendril": segments_per_tendril,
        "tendril_mode": effective_mode,
        "branching_probability": float(effective_branching),
        ...
        "intended_tendril_count": int(effective_count),
